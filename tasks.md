...
- [x] **Test `estimateCost` returns correct cost for known model** -- Estimate cost for a known model with specific token counts and assert correct `inputCost`, `outputCost`, `totalCost`. | Status: done
...
- [x] **Test `estimateCost` uses base pricing below tier threshold** -- For gemini-2.5-pro with `inputTokens: 100000`, assert the base rate ($1.25/MTok) is used. | Status: done
- [x] **Test `estimateCost` `currency` field is always `'USD'`** -- Assert the `currency` field on the returned `CostEstimate` is `'USD'`. | Status: done

### 6c: `listProviders` Tests

- [x] **Test `listProviders` returns all provider IDs** -- Assert the returned array contains `'openai'`, `'anthropic'`, `'google'`, `'meta'`, `'mistral'`, `'cohere'`. | Status: done
- [x] **Test `listProviders` returns non-empty array** -- Assert length is at least 6. | Status: done
- [x] **Test `listProviders` returns strings** -- Assert every element is a string. | Status: done

### 6d: `listModels` Tests

- [x] **Test `listModels` returns all models with no options** -- Assert result is a non-empty array of `ModelSummary` objects. | Status: done
- [x] **Test `listModels` filters by provider** -- Call with `{ provider: 'openai' }` and assert all returned models have `provider: 'openai'`. Assert count matches expected (11 for OpenAI). | Status: done
...
- [x] **Test `getModelInfo` returns `ModelInfo` with aliases** -- Call for a model that has aliases and assert `aliases` is a non-empty array. | Status: done
- [x] **Test `getModelInfo` returns `undefined` for unknown model** -- Assert result is `undefined` for a non-existent model. | Status: done
- [x] **Test `getModelInfo` includes `launchDate` when available** -- Assert `launchDate` field is present for models that have it in the registry data. | Status: done
...
- [x] **Test `price` command with valid model** -- Run `model-price-registry price openai gpt-4o`, assert exit code 0 and output contains pricing information. | Status: done
- [x] **Test `price` command with unknown provider** -- Run `model-price-registry price unknown-provider unknown-model`, assert exit code 1. | Status: done
- [x] **Test `price` command with `--format json`** -- Run with `--format json` flag and assert output is valid JSON. | Status: done
- [x] **Test `estimate` command** -- Run `model-price-registry estimate openai gpt-4o --input 1000 --output 500`, assert exit code 0 and output contains cost estimate. | Status: done
